from pathlib import Path
from app.spline import SplineManager

class ImageManager:
    def __init__(self, image_dir: str, spline_manager):

        self.image_dir = Path(image_dir)
        all_images = self._index_images(self.image_dir)

        self.images_to_spline = [img for img in all_images if not spline_manager.spline_exists(img)]

        self.num_images = len(all_images)
        self.num_previously_splined = len(all_images) - len(self.images_to_spline)

        # dictionary of {image: previous_image} (first image will be mapped to None)
        shifted_right = [None] + self.images_to_spline[:-1]
        self._previous = dict(zip(self.images_to_spline, shifted_right))

        # dictionary of {image: next_image} (last image will be mapped to None)
        shifted_left = self.images_to_spline[1:] + [None]
        self._next = dict(zip(self.images_to_spline, shifted_left))

    def get_first_image(self):
        if len(self.images_to_spline) == 0:
            return None
        return self.images_to_spline[0]

    def get_next_image(self, current_image):
        return self._next[current_image]

    def get_previous_image(self, current_image):
        return self._previous[current_image]

    @staticmethod
    def _index_images(image_dir):
        assert image_dir.exists()
        assert image_dir.is_dir()

        images = list(image_dir.glob("*.png")) + list(image_dir.glob("*.jpg"))
        images = sorted(list(images))
        #assert len(images) > 0

        return [img.name for img in images]
